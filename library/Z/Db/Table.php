...
class Z_Db_Table extends Zend_Db_Table_Abstract
{
   ...
   * @return <array>
   */
   public function fetchPairs($keys = NULL,$where=NULL,$order=NULL,$firstzero=false)
  {
   ...
   public function fetchTriples($keys = NULL,$where=NULL,$order=NULL)
  {
    if ($keys===NULL) $keys = array('id','title');
    if ($where===NULL) $where = array();
    $select = $this->select()->from($this,$keys);
    ...
    if (!empty($order)) $select->order($order);
    $ret = $this->getAdapter()->fetchAssoc($select);
    return $ret;
  }
